...
async def chat_node(state: ChatState, config: RunnableConfig, llm_with_tools):
    ...
    system_messages = [SystemMessage(content=(
        ...
        "\n\n"
        "IMPORTANT - handling time-sensitive information: "
        "Your training data has a fixed cutoff date and may be outdated. "
        "Before making any claim about a company's status (public/private, IPO, ticker symbol, "
        "stock price, recent news, leadership changes, acquisitions, or any event that may have "
        "occurred after mid-2025), you MUST call search_tool to verify the current facts. "
        "Never assume a company is still private, still public, or still under the same leadership "
        "without searching first. "
        "For stock price requests, first use search_tool to confirm the current ticker symbol, "
        "then call stock_get_price with that ticker."
    ))]

...
load_dotenv()  # safety net when running as a subprocess
...
async def stock_get_price(params: StockPriceInput) -> str:
    ...
    if not api_key:
        return json.dumps({"error": "ALPHA_VANTAGE_API_KEY is not configured."})
    ...
    # Surface real API-level errors instead of silently returning "no data"
